# SystemPrompt.md — AutoRadar 产品展示页生成指令

> 读取顺序：`CLAUDE.md` → `DESIGN.md` → 本文件 → `src/data/products.ts`
...
2. All product data is **hardcoded** as a static `PRODUCTS` array in `App.tsx`. Product images are served from `https://vibe-view-demo-site.vercel.app/products/{id}.png`. No external API calls, no environment variables required.
